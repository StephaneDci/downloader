...
class TestDownloaderMocked(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.srcpage = TestDownloaderMocked.read_file_to_html(
            'data/test_www.audiolitterature.com_michel-onfray-traite-datheologie_.html')
        cls.Page = Page(url=cls.url, directory=cls.directory, pattern=cls.pattern, proxies=global_proxy)
    ...
    def test_properties_ressource2(self):
        p = Page(url="https://supersiteaudio.com/monartiste", directory="", pattern=r"http\S+mp3", proxies=global_proxy)
    ...
    def test_properties_ressource3(self):
        p = Page(url="https://supersiteaudio.com/artiste1/listings/1/", directory="", pattern=r"http\S+mp3", proxies=global_proxy)
    ...
    def test_properties_ressource_size(self):
        p = Page(url="http://myurl.com", directory="myrep", pattern="nopattern", proxies=global_proxy)
    ...
    def test_add_download_Page(self):
        ...
        # Test doublon
        p5 = Page(url="url3", pattern='pattern3', proxies=global_proxy)
        fd.add_download_page(p5)
        self.assertEqual(4, len(fd.pages))

    def test_doublon_dans_global_ressources(self):
        p1 = Page(url="http://myurl1.com", directory="myrep1", pattern="mp3", proxies=global_proxy)
        p2 = Page(url="http://myurl2.com", directory="myrep2", pattern="mp3", proxies=global_proxy)
