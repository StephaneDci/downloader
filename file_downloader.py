...
# Utilisation des Dataclasses et de la programmation Objet.
# Auteur : SDI
# Date   : 15/02/2020
# Objectif : educationnal purpose only. Merci de respecter les copyrights.
# Python >= 3.8 + modules dans les requirements
# ------------------------------------------------------------------------------------------------------
from __future__ import annotations                                                      # Dépendances circulaires
import os                                                                               # Os
import re                                                                               # Regex
from pathlib import Path                                                                # Chemin Posix
from urllib.parse import urlparse                                                       # Parsing des urls
from typing import List, Set, Union                                                     # Typage
from tqdm import tqdm                                                                   # Bar d'avancement
...
from downloader.utils.network import make_get_requests                                  # Bibliothèque network
from downloader.utils.system import create_target_directory, verify_if_file_exists      # Bibliothèque system
from downloader import logger, global_proxy, session                                    # Variables globales
...
class RessourceFile:
    ...
    link: str                                                                           # lien vers la ressource
    page: Page                                                                          # Page contenant la ressource
    headers: {} = field(repr=False, default_factory=lambda: {})                         # les headers de la ressource
    downloaded: bool = field(default=False)                                             # si téléchargement ok

    # ------------------------------------------------------------------------------
    # Déclaration des propriétés de la classe, éléments déductibles des attributs.
    ...
    # -----------------------------------------------------------------------------------------
    # Réecriture de l'opérateur d'égalité des ressources (se base uniquement sur le lien http)
    # -----------------------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------------------------------
# Classe WebRessourcePattern, permettant de définir des patterns de recherche de motifs des ressources
...
class Page:
    ...
    def size_downloaded(self):
        return sum(r.size for r in self.ressources if r.downloaded)
    ...
    def set_directory(self, directory: str) -> str:
        ...
        return str(Path(Page.DEFAULT_DIR + '/' + urlparse(self.url).path[1:].rstrip('/'))) \
    ...
    def process(self):
        ...
        try:
            ...
            self.srcpage = make_get_requests(self.url, proxies=self.proxies, session=session).text
    ...
    def set_unit_ressource_infos(self, res: RessourceFile):
        head = make_get_requests(res.link, verb='HEAD', proxies=self.proxies, session=session)
    ...
    def __repr__(self):
        return f"<Page>(url:'{self.url}', directory:'{self.directory}', pattern:'{self.pattern}', " \
               f"proxies: {self.proxies}, nbr_ressources: {self.nbr_ressources}, " \
               f"size_ressources: {self.size_ressources}, size_downloaded: {self.size_downloaded}, " \
               f"nbr_downloads: {self.nbr_download})"
...
# --------------------------------------------------------------------------
# Classe FileDownloader. Contient une ou plusieurs Pages.
# --------------------------------------------------------------------------
...
class FileDownloader:
    def __init__(self, page: Union[Page, List[Page]], nmax_threads: int = 10):
        ...
        self.add_download_page(page)         # Ajout des pages.
    ...
    def download_unit(self, res: RessourceFile) -> str:
        ...
        if not downloaded:
            rep = make_get_requests(res.link, stream=True, proxies=res.page.proxies, session=session)
...
if __name__ == '__main__':
    ...
    p1 = Page(url="https://www.audiolitterature.com/michel-onfray-traite-datheologie/", pattern="mp3", proxies=global_proxy)

    # Sans préciser le répertoire celui-ci sera déduit de l'url
    p2 = Page("https://www.audiolitterature.com/michel-onfray-nietzsche-en-quatre-questions/", "mp3", proxies=global_proxy)
    ...
    p3 = Page("https://www.audiolitterature.com/michel-onfray-breve-encyclopedie-du-monde-saison-4/", "mp3", proxies=global_proxy)
    p4 = Page("https://www.audiolitterature.com/michel-onfray-le-souci-des-plaisirs/", "mp3", proxies=global_proxy)
    p5 = Page("https://www.audiolitterature.com/michel-onfray-le-pur-plaisir-dexister/", "mp3", proxies=global_proxy)
    lst = [p3, p4, p5]
    fd.add_download_page(lst)

    # Ou ajout des pages comme ceci evidemment
    fd.add_download_page([Page("https://www.audiolitterature.com/michel-onfray-breve-encyclopedie-du-monde-saison-3/", "mp3", proxies=global_proxy),
                         Page("https://www.audiolitterature.com/michel-onfray-breve-encyclopedie-du-monde-saison-2/", "mp3", proxies=global_proxy),
                         Page("https://www.audiolitterature.com/michel-onfray-breve-encyclopedie-du-monde-saison-1/", "mp3", proxies=global_proxy)])
